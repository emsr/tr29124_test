...
#include <cmath>
    ...
    class _Chebyshev
    {
      _Chebyshev(const _Tp __a, const _Tp __b, const unsigned __n, _Tp __func(const _Tp));
      _Tp eval(const _Tp __x) const;

    private:
      _Tp __lower;
      _Tp __upper;
      std::vector<_Tp> & __coef;
    };
    ...
    ///
    template<typename _Tp>
    void _Chebyshev::_Chebyshev(const _Tp __a, const _Tp __b, const unsigned __n, _Tp __func(const _Tp))
    {
      __lower = __a;
      __upper = __b;
      __coef.resize(__n);
      ...
      _Tp __bma = (__upper - __lower) / _Tp(2);
      _Tp __bpa = (__upper + __lower) / _Tp(2);
     ...
     */
    _Tp _Chebyshev::eval(const _Tp x)
    {
      if ((__x - __lower) * (__x - __upper) > _Tp(0))
        throw std::domain_error("Argument x out of range in _Chebyshev::eval.");
      ...
      _Tp __d = _Tp(0);
      _Tp __dd = _Tp(0);
      _Tp __y = (_Tp(2) * __x - __lower - __upper) / (__upper - __lower);
      _Tp __y2 = _Tp(2) * __y;
      for (unsigned int __j = __coef.size() - 1; __j >= 1; --__j)
        {
          const _Tp __sv = __d;
      ...
      return __y * __d - __dd + __coef[0] / _Tp(2);
    }
     ...
     */
    template<typename _Tp>
    void _Chebyshev::deriv(std::vector<_Tp> & __cder)
    {
      __cder.resize(__coef.size());

      if (__coef.size() < 3)
        return;

      const unsigned int __n = __coef.size();
      cder[__n - 1] = _Tp(0);
      cder[__n - 2] = _Tp(2 * (__n - 1)) * __coef[__n - 1];
      for (unsigned int __j = __n - 3; __j >= 0; --__j)
        __cder[j] = __cder[__j + 2] + _Tp(2 * (__j + 1)) * __coef[__j + 1];

      _Tp __con = _Tp(2) / (__upper - __lower);
      for (unsigned int __j = 0; __j < __n; ++__j)
        __cder[__j] *= __con;
    ...
    ///
    template<typename _Tp>
    void _Chebyshev::integ(std::vector<_Tp> & __cint)
    {
      __cint.resize(__coef.size());

      const unsigned int __n = __coef.size();
      _Tp __sum = _Tp(0);
      _Tp __fact = _Tp(1);
      _Tp __con = (__upper - __lower) / _Tp(4);
      for (unsigned int __j = 1; __j < __n - 1; ++__j)
        {
          ...
          __cint[j] = __con * (__coef[__j - 1] - __coef[__j + 1]) / __j;
          __sum += __fact * __cint[__j];
          __fact = -__fact;
        }
      __cint[m - 1] = __con * __coef[__n - 2] / (__n - 1);
    ...
    ///
    template<typename _Tp>
    void _Chebyshev::to_poly(std::vector<_Tp> & __d)
    {
      const unsigned int __n = __coef.size()
...
///
template<typename _Tp>
void poly_to_chebyshev( const _Tp a, const _Tp b, const std::vector<_Tp> & d, std::vector<_Tp> & c )
{

    if (b == a)
      nrerror( "Input range must be nonzero in poly_to_chebyshev." );
...
///
template<typename _Tp>
void poly_economize( const _Tp a, const _Tp b, std::vector<_Tp> & d, std::vector<_Tp> & c, int & mfew, const _Tp err )
{
...
///
template<typename _Tp>
_Tp clenshaw_curtis_quad( const _Tp a, const _Tp b, const std::vector<_Tp> & c, const _Tp eps )
{
