...
C  this version written up       in    CPC 47 (1987) 245-257 ABBM      C
C                      corrected in    CPC 159(2004) 243-244           C
C                                                                      C
...
C
      ...
      LOGICAL IFIP
      REAL*8 ABSC,ACCUR,ACC,ACCDEF,ACCLOG,GAM(NGAM),CSC(NCSC),AIMAG,
...
C
           ...
           F2 = CMPLX(ZERO,TWO) / (F1 * (PQ1-PQ2))
            SG=SIGN(1d0,REAL(X))
            SL = CMPLX(-TWO*REAL(Z),-SG*YL0*PI-C2-C2)
              RAT = ZERO
