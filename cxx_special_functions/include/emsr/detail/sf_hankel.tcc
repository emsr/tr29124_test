...
#ifndef SF_HANKEL_TCC
#define SF_HANKEL_TCC 1
...
namespace detail
{
  ...
  template<typename Tp>
		    ...
		    std::complex<Tp>& zetam3hf, std::complex<Tp>& zetrat)
    {
      ...
      const auto s_sqrt2 = emsr::sqrt2_v<Tp>;
      ...
      auto w = Cmplx{Tp{1}} - emsr::safe_sqr(zhat);
      w = std::sqrt(w);
      p = Tp{1} / w;
      p2 = p * p;

      nup2 = emsr::safe_sqr(nu);
      num2 = Tp{1} / nup2;
  ...
  template<typename Tp>
	       ...
	       std::complex<Tp>& argp, std::complex<Tp>& argm)
    {
      ...
      const auto s_sqrt3d2 = emsr::sqrt3_div_2_v<Tp>;
      const auto expp = Cmplx{-0.5L,  s_sqrt3d2};
      const auto expm = Cmplx{-0.5L, -s_sqrt3d2};

      try
	{
	  argm = emsr::safe_div(num2d3, zeta);
	  argp = expp * argm;
	  argm = expm * argm;
	}
      catch (...)
  ...
  template<typename Tp>
    void
    hankel_uniform_outer(const std::complex<Tp>& nu, const std::complex<Tp>& z, Tp /*eps*/,
   ...
   * @return A struct containing the cylindrical Hankel functions
   *         of the first and second kinds and their derivatives.
   */
  template<typename Tp>
    emsr::cyl_hankel_t<std::complex<Tp>, std::complex<Tp>, std::complex<Tp>>
    hankel_uniform_olver(std::complex<Tp> nu, const std::complex<Tp>& z)
    {
      using namespace std::complex_literals;
      using Cmplx = std::complex<Tp>;

      using hank_t = emsr::cyl_hankel_t<Cmplx, Cmplx, Cmplx>;

      const auto s_pi = emsr::pi_v<Tp>;
      const auto s_sqrt_3 = emsr::sqrt3_v<Tp>;
      const Cmplx s_j{1il};
   ...
   *
   * @param[in] nu The order for which the cylindrical Hankel function
   *          of the first kind is evaluated.
   * @param[in] z  The argument at which the cylindrical Hankel function
   *          of the first kind is evaluated.
   ...
   *
   * @param[in] nu The order for which the cylindrical Hankel function
   *          of the second kind is evaluated.
   * @param[in] z  The argument at which the cylindrical Hankel function
   *          of the second kind is evaluated.
   ...
   *
   * @param[in] nu The order for which the cylindrical Bessel function
   *          is evaluated.
   * @param[in] z  The argument at which the cylindrical Bessel function
   *          is evaluated.
   ...
   *
   * @param[in] nu The order for which the cylindrical Neumann function
   *          is evaluated.
   * @param[in] z  The argument at which the cylindrical Neumann function
   *          is evaluated.
   ...
   *        and their derivatives.
   *
   * @param[in] n The order for which the spherical Hankel functions
   *                are evaluated.
   * @param[in] z The argument at which the spherical Hankel functions
   *                are evaluated.
   * @return  A struct containing the spherical Hankel functions
   *          of the first and second kinds and their derivatives.
   */
  template<typename Tp>
    emsr::sph_hankel_t<unsigned int, std::complex<Tp>, std::complex<Tp>>
    sph_hankel(unsigned int n, const std::complex<Tp>& z)
    {
      using Cmplx = std::complex<Tp>;
      using hank_t = emsr::sph_hankel_t<unsigned int, Cmplx, Cmplx>;
      const auto s_pi = emsr::pi_v<Tp>;
      Cmplx nu(n + Tp{0.5});
      auto hank = hankel(nu, z);
   ...
   *
   * @param[in] n The order for which the spherical Hankel function
   *                of the first kind is evaluated.
   * @param[in] z The argument at which the spherical Hankel function
   *                of the first kind is evaluated.
   * @return The complex spherical Hankel function of the first kind.
   */
  template<typename Tp>
    std::complex<Tp>
    sph_hankel_1(unsigned int n, const std::complex<Tp>& z)
    {
  ...
  template<typename Tp>
    std::complex<Tp>
    sph_hankel_2(unsigned int n, const std::complex<Tp>& z)
    {
      auto hank = sph_hankel(n, z);
      return hank.h2_value;
    }
   ...
   *
   * @param[in] n The order for which the spherical Bessel function
   *                is evaluated.
   * @param[in] z The argument at which the spherical Bessel function
   *                is evaluated.
   ...
   *
   * @param[in] n The order for which the spherical Neumann function
   *                is evaluated.
   * @param[in] z The argument at which the spherical Neumann function
   *                is evaluated.
