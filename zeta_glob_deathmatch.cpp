/*
$HOME/bin/bin/g++ -std=c++17 -Wall -Wextra -Wno-psabi -g -I. -o zeta_glob_deathmatch zeta_glob_deathmatch.cpp -lquadmath -Lwrappers/debug -lwrap_gsl
LD_LIBRARY_PATH=$HOME/bin/lib64:$LD_LIBRARY_PATH ./zeta_glob_deathmatch > zeta_glob_deathmatch.txt

$HOME/bin/bin/g++ -std=c++17 -Wall -Wextra -g -I. -o zeta_glob_deathmatch zeta_glob_deathmatch.cpp -lquadmath -Lwrappers/debug -lwrap_gsl
...
#define _GLIBCXX_MATH_NS std
    ...
    __riemann_zeta_glob_new(_Tp __s)
    {
      ...
      const _Tp __eps = std::numeric_limits<_Tp>::epsilon();
      //  Max e exponent before overflow.
      const _Tp __max_binom = std::exp(std::numeric_limits<_Tp>::max_exponent10
                               * std::log(_Tp(10)) - _Tp(1));
          ...
          for (unsigned int __j = 1; __j <= __i; ++__j)
            {
              ...
              __term += __binom * std::pow(_Tp(1 + __j), -__s);
            }
          if (__punt)
            break;
          __term *= __num;
          ...
          for (unsigned int __j = 1; __j <= __i; ++__j)
            {
              ...
              __term += __binom * std::pow(_Tp(__a + __j), -__s);
            }
          if (__punt)
            break;
          __term *= __num;
