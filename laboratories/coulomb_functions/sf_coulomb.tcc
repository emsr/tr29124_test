...
//printf("dr = %f; di = %f\n", std::real(d), std::imag(d));
//printf("dp = %f; dq = %f\n", std::real(dpq), std::imag(dpq));
...
template<typename Tp>
  emsr::Coulomb<Tp>
  coulomb_wave_FG(Tp lam_F, Tp eta, Tp rho, int k_lam_G) // lam_G = lam_F - k_lam_G
  {
    const auto s_pi = emsr::pi_v<Tp>;
...
//printf("F_sign_lam_F = %f\n", F_CF1.first);
//printf("Fp_over_F_lam_F = %f\n", Fp_over_F_lam_F);
//printf("span = %d\n", span);
...
//printf("F_lam_min_unnorm = %f\n", F_lam_min_unnorm);
//printf("Fp_lam_min_unnorm = %f\n", Fp_lam_min_unnorm);
//printf("lam_min = %f\n", lam_min);
