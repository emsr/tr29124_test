/*
g++ -std=c++20 -o rational_fit rational_fit.cpp
*/
             ...
             std::vector<Real>& coeff, Real& dev)
{
  ...
  for (int iter = 0; iter <= max_iters; ++iter)
  {
    ...
    std::vector<std::vector<Real>> u(num_points, std::vector<Real>(num_coeffs));
    for (int i = 0; i < num_points; ++i)
    {
      auto power = wt[i];
      bb[i] = power * (fs[i] + std::copysign(e, err[i]));
      for (int j = 0; j < deg_numer + 1; ++j)
      {
        u[i][j] = power;
        power *= xs[i];
      }
    ...
    emsr::sv_decomposition<MatTp> svd(std::size_t(num_points), std::size_t(num_coeffs), u);
