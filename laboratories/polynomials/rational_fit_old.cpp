...
std::vector<Real> w(num_coeffs);
MatTp v(num_coeffs, std::vector<Real>(num_coeffs));
emsr::sv_decomp(num_points, num_coeffs, u, w, v);
emsr::sv_backsub(num_points, num_coeffs, u, w, v, bb, coeff_temp);
      ...
      for (int i = 1; i <= 100; ++i)
        {
          ...
          //auto r = rat(x);
          auto r = rat.numer(x) / (Real{1} + x * rat.denom(x));
          auto f = func(x);
          std::cout << ' ' << x << ' ' << r << ' ' << f << ' ' << r - f << '\n';
        }
    }
  ...
  for (int i = 2; i < 100; ++i)
    {
      ...
      auto x = 0.01 * i * pi;
      auto xp = 0.01 * (i + 1) * pi;
      auto delm = rat.numer(xm) / (1.0 + xm * rat.denom(xm)) - fun(xm);
      auto del = rat.numer(x) / (1.0 + x * rat.denom(x)) - fun(x);
      auto delp = rat.numer(xp) / (1.0 + xp * rat.denom(xp)) - fun(xp);
