...
{---------------------------------------------------------------------------}
function sfc_igprefix(a,x: extended): extended;
  {-return dax = x^a*exp(-x)/gamma(a)}
var
  mu,d,t: extended;
begin
  mu := (x-a)/a;
  {Calculate dax = x^a*exp(-x)/gamma(a+1). This is the most sensible part for}
  {controlling the rounding errors occurring in the incomplete gamma functions}
  {This is function daxw cracked-down for TP5's brain-damaged FPU usage}
  if (a > 8.0) and (abs(mu) < 0.99) then begin
    t := ln1pmx(mu);
    d := a*t;
    t := 0.5*ln(a/TwoPi);
    d := d + t;
    ...
    if (a > 8.0) and (x > 10.0) then begin
      t := mu - ln(x/a);
      d := a*t;
      t := sfc_lngcorr(a);
      d := d + t;
...
var
  ...
  ok: boolean;
begin
  if (x<0.0) or IsNanOrInf(x) or IsNanOrInf(a) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfc_igamma := NaN_x;
...
begin
  w := a*ln(x);
  if abs(x) < 0.5 then z := b*ln1p(-x)
  else z := b*ln(y);
  t := sfc_lnbeta(a,b);
  t := w+z-t;
  sfc_ibetaprefix:= exp(t);
end;
