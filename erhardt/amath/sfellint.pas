...
{$define use_bulirsch}  {sfc_EllipticPiC/CPi with sfc_cel can be used by TP5}
...
const
  ...
  c1 :extended = 1.0/24.0;
  c2 :extended = 0.1;
  c3 :extended = 3.0/44.0;
  c4 :extended = 1.0/14.0;
var
...
const
  ...
  c1 :extended = -3.0/14.0;
  c2 :extended = 1.0/6.0;
  c3 :extended = 9.0/22.0;
  c4 :extended = 3.0/26.0;
  c5 :extended = 9.0/88.0;
  c6 :extended = 9.0/52.0;
var
  l,m,dx,dy,dz,ea,eb,ec,ed,ef,p4,rx,ry,rz,s,sn: extended;
begin
  if IsNanOrInf(x) or IsNanOrInf(y) or IsNanOrInf(z) or (maxx(x,y) <= 0.0) or (z <= 0.0) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfc_ell_rd := NaN_x;
...
begin
  ...
  repeat
    h := m;
    m := kc + m;
    if abs(h-kc) <= 2e-10*h then done := true
    else begin
      kc:= sqrt(h*kc);
      m := 0.5*m;
    end;
...
begin
  if nu=0.0 then sfc_EllipticCPi := sfc_EllipticCK(k)
    ...
    {$ifdef use_bulirsch}
      sfc_EllipticCPi := sfc_cel(k,1.0-nu,1.0,1.0);
    {$else}
      k := k*k;
      {See Carlson [12], (4.3) with sin()=1, cos()=0}
      sfc_EllipticCPi := sfc_ell_rf(0.0,k,1.0) + nu*sfc_ell_rj(0.0,k,1.0,1.0-nu)/THREE;
    {$endif}
  end;
end;
...
begin
  if nu=0.0 then sfc_EllipticPiC := sfc_EllipticK(k)
    ...
    {$else}
      {See Carlson [12], (4.3) with sin()=1, cos()=0}
      a := (1.0+k)*(1.0-k);
      sfc_EllipticPic := sfc_ell_rf(0.0,a,1.0) + nu*sfc_ell_rj(0.0,a,1.0,1.0-nu)/THREE;
    {$endif}
  end;
end;
...
procedure special_reduce_modpi(z: extended; var a,x: extended; var near_pi2: boolean);
  ...
  { Internal special code for sfc_ellint_1/2/3 and sfc_hlambda}
var
  t: extended;
begin
  t := abs(z);
...
begin
  ...
  if x=0.0 then begin
    ...
    for i:=0 to NA do begin
      ...
      n[i]:= mc;
      c   := 0.5*(a+mc);
      if abs(a-mc) <= ca*a then goto done;
      mc := a*mc;
      a  := c;
    end;
done:
...
begin
  ...
  if k<=k1 then begin
    ...
    a := exp(PiSqr/r);
    b := PiSqr*t/sqr(r);
    sfc_nome := a*(1.0 - 0.5*b*(1.0 + (13.0/32.0)*t));
  end
  else begin
