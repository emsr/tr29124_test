...
(*
{---------------------------------------------------------------------------}
function cl2_mf(x: extended): extended;
  {-Return Clausen's integral, Cl_2(x) = integral(-ln(2*sin(t/2)),t=0..x) = Im(Li_2(exp(ix)))}
var
  t: extended;
  n: boolean;
const
  CA: array[0..15] of extended = (
        ...
        0.3e-19);
const
  xsmall = 7.314590396335798E-10; {Pi*sqrt(eps_x/2)}
begin
  {Ref: MISCFUN [22], function clausn}
  {NOTE: This is a Pascal reference implementation of the MISCFUN function.}
  {      The relative errors are large near the odd zeroes.}
  n := x<0.0;
  {Range reduction: cl2(x+2*n*Pi) = cl2(x)  and  cl2(Pi+x) = -cl2(Pi-x)}
  x := rem_2pi(abs(x));
  if x>Pi then begin
    x := (6.283172607421875-x)+0.1269975771147692529e-4;
    n := not n;
  end;
  if x<=0.0 then cl2_mf := 0.0
  else begin
    if x<xsmall then t := x*(1.0-ln(x))
    else begin
      t := 2.0*(sqr(x)/PiSqr-0.5);
      if t>1.0 then t:=1.0;
      t := x*(CSEvalX(t,CA, 16) - ln(x));
    end;
    if n then cl2_mf := -t else cl2_mf := t;
  end;
end;
*)
...
{---------------------------------------------------------------------------}
function polylogseries(n: integer; x: extended): extended;
...
{---------------------------------------------------------------------------}
function polylogneg(n: integer; x: extended): extended;
...
begin
  if x = 1.0 then polylogneg := PosInf_x
  ...
  else if x=-1.0 then polylogneg := -sfc_etaint(n)
    ...
    until done;
    polylogneg := sfc_fac(-n)*power(-lx, n-1) - s;
  end
  else begin
...
begin
  ...
  if IsNan(x) or ((n>0) and (x>1.0)) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfc_polylog := Nan_x;
  ...
  if x=0.0 then begin
    ...
    if x=1.0 then begin
      {$ifopt R+}
        if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
      {$endif}
      sfc_polylog := PosInf_x;
    end
    else sfc_polylog := polylogneg(n,x);
    exit;
  end
  ...
  {here x<=1 and n>2}
  if x=1.0 then sfc_polylog := sfc_zetaint(n)
  else if x=-1.0 then sfc_polylog := -sfc_etaint(n)
  else if x < -1.0 then begin
    ...
    repeat
      ...
      h := sfc_zeta(n-j)*p;
      s := s + h;
      if abs(h) < eps_x*abs(s) then begin
        sfc_polylog := s;
        exit;
      end;
      inc(j,2);
...
function sfc_polylogr(s, x: extended): extended;
  {-Return the polylogarithm Li_s(x) of real order; s >= -1, |x|<=1, x<>1 if s=1}
begin
  if IsNanOrInf(s) or IsNanOrInf(x) or (s < -1.0) or (abs(x) > 1.0) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfc_polylogr := NaN_x;
  ...
  if x=0.0 then sfc_polylogr := 0.0
  else if abs(x)=1.0 then begin
    if x=1.0 then sfc_polylogr := sfc_zeta(s)
    else sfc_polylogr := -sfc_eta(s)
  end
  else if s>=64.0 then sfc_polylogr := x
  else begin
    if s>=40.5 then sfc_polylogr := x*(1.0 + x*exp2(-s))
    else begin
      if frac(s)=0.0 then sfc_polylogr := sfc_polylog(trunc(s),x)
      else begin
    ...
    if x <= 0.5 then begin
      ...
      if abs(omega) <= MinExtended then begin
        ...
        if i=0 then begin
          ...
          {$ifndef V7Plus}goto{$endif} continue;
        end
        else begin
          if RTE_NoConvergence>0 then RunError(byte(RTE_NoConvergence));
          exit;
...
var
  z: extended;
begin
  if IsNanOrInf(s) or IsNanOrInf(x) or (s<0.0) or (abs(x) > 1.0) then begin
    {$ifopt R+}
      if RTE_ArgumentRange>0 then RunError(byte(RTE_ArgumentRange));
    {$endif}
    sfc_lchi := NaN_x;
  ...
  if x=0.0 then sfc_lchi := 0.0
  else if s>=22.8 then begin
    ...
    if s=0.0 then sfc_lchi := x/((1.0+x)*(1.0-x))
    else if s=1.0 then sfc_lchi := arctanh(x)
    else sfc_lchi := sfc_lerch(x*x,s,0.5)*x*exp2(-s);
  end;
end;
