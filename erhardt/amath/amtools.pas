...
procedure intde(f: TFuncX; a, b, eps: extended; var result, abserr: extended;
  ...
  { neval  - number of integrand evaluations               }
  { ier    - 0: OK, 1: eps < 8*eps_x, 2: roundoff problems }
...
procedure intdeo(f: TFuncX; a, omega, eps: extended; var result, abserr: extended;
  ...
  { abserr - estimate of the absolute error, if ier=0 or 3 }
  { neval  - number of integrand evaluations               }
  { ier    - 0: OK, 1: eps < 8*eps_x, eps>1, or omega=0    }
  {          2: roundoff problems (function argument = a)  }
  {          3: max. iterations, result/abserr have values }
  {#F}
...
procedure levinu1(an: extended; n,nmax: integer;
               ...
               {$else}
                  var vqnum, vqden{: array of extended};
               {$endif}
...
{$ifndef CONST}
var
  qnum: TExtVector absolute vqnum;
  qden: TExtVector absolute vqden;
{$endif}
begin
...
const
  w0: extended =   3956.0 / 14175.0; {Newton-Cotes weights multiplied by 8}
  w1: extended =  23552.0 / 14175.0;
  w2: extended =  -3712.0 / 14175.0;
  w3: extended =  41984.0 / 14175.0;
  w4: extended = -18160.0 / 14175.0;
...
begin
  ...
  if (eps < 8.0*eps_x) or (eps > 1.0) or (omega=0.0) then begin
    ier := 1;
    exit;
  end;
