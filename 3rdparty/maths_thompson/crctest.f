      program crcwt  
c
c     test program for crcwfn :  to produce test output for cpc paper
c
      implicit real*8(a-h,o-z)
      parameter(mch=4,kmax=4,maxv=100000,lmax1=27)
c
      real*8 eta(mch),ak2(mch),coupl(mch,mch,kmax),fcc(mch,mch,2),
     * gcc(mch,mch,2),v(maxv),accrcy,rin(2),rout,f(lmax1),g(lmax1),
     * fp(lmax1),gp(lmax1)
      integer lval(mch)
...
c
      ...
      rin(2)=25.5d0
      rout=200.0d0
      write(6,1002) rin(1),rin(2),rout
      write(6,1003) accrcy,switch,maxv
      write(6,1004) nch,mch,kmax
      write(6,1005) correc,deriv,cfg
1001  format(/,/,'TEST RUN OUTPUT: CRCWFN',/)
1002  format('rin(1)= ',f7.2,' fm',2x,'rin(2)= ',f7.2,' fm',
     *      2x,'rout=   ',f7.2,' fm')
1003  format('accrcy= ',f8.3,4x,'switch= ',f7.1,' fm',2x,
     *      'maxv=   ',i7) 
1004  format('nch=    ',i2,10x,'mch=    ',i2,10x,'kmax=   ',i2)
1005  format('correc= ',l2,10x,'deriv=  ',l2,10x,'cfg=    ',l2)
c
      lval(1)=20
      lval(2)=18
      lval(3)=20
...
c
      write(6,1006)
      do 10 i=1,4
10       write(6,1007) i,lval(i),eta(i),ak2(i)
1006  format(/,'Channel',4x,'L (hbar)',5x,'Eta',9x,'k**2',/)
1007  format(3x,i3,4x,i4,5x,2(f10.5,3x))
c
      do 20 i=1,4
         do 30 j=1,4
             do 30 k=1,4
 30             coupl(i,j,k)=0.d0
         coupl(i,i,1)= 2.d0*eta(i)*sqrt(ak2(i))
         coupl(i,i,2)= lval(i)*(lval(i)+1)            
 20      continue
c
      write(6,1008)
      do 25 i=1,4
 25          write(6,1012) i,i,(coupl(i,i,k),k=1,4)
      write(6,1013)
1008  format(/,/,'COUPL ARRAY:',/,/,'Multipolarity (k-1) = 0        1'
     *       ,11x,'2',11x,'3',/)
1012  format('coupl(',i1,',',i1,',k)=',4(f10.3,2x))
1013  format(/,'All off-diagonal elements zero',/,/)
c
      write(6,1009)
      write(6,1010)
      do 200 nc =1,nch
        ...
        i = lval(nc)+1
	write(6,1011) nc,f(i),ak*fp(i),g(i),ak*gp(i)
200     continue
1009  format('(1) standard Coulomb functions evaluated at Rin',/)
1010  format(12x,'F',9x,'  dF/dr',10x,'G',9x,'  dG/dr')
1011  format(x,'ch',i1,2x,4(f11.7,3x))
c
c
      call crcwfn(nch,ak2,kmax,coupl,rin,rout,
     *    accrcy,v,fcc,gcc,mch,maxv,correc,deriv,cfg,switch)
c
      write(6,1014)
c
      do 100 i=1,4
100      write(6,1011) i,fcc(i,i,1),fcc(i,i,2),gcc(i,i,1),gcc(i,i,2)
      write(6,1015)
1014  format(/,'(2) Coulomb wavefunctions integrated in ',
     *     'from Rout with no coupling',/)
1015  format(/,'All off-diagonal elements zero ')
c
      coupl(1,2,3)= 1755.d0
      coupl(1,3,3)= -1471.d0
      coupl(1,4,3)= 1842.d0
...
c
      do 120 i=1,4
         do 120 j=1,4
 120          write(6,1012) i,j,(coupl(i,j,k),k=1,4)
c
      call crcwfn(nch,ak2,kmax,coupl,rin,rout,
     *    accrcy,v,fcc,gcc,mch,maxv,correc,deriv,cfg,switch)
c
      ...
      write(6,1016) i
      do 500 j=1,4
500      write(6,1017) fcc(j,i,1),fcc(j,i,2),gcc(j,i,1),gcc(j,i,2)
c
c               
1016  format('channel ',i2)
1017  format(6x,4(f11.7,3x))
1018  format(/,/,'(3) coupled Coulomb wavefunctions integrated ',
     *    'in from Rout',/)
      end    
c
