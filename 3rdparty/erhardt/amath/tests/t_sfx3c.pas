...
{$ifdef VER5X}
const
  NE  = 4*10;         {sfc_ibetaprefix has no Lanczos support for TP50}
  NE2 = 4*60;
  NE3 = 4*600;
{$else}
const
...
{$ifdef VER50}
  ...
  y := poisson_pmfx(mu,k);
  testrel(10, 150, y, f, cnt,failed);
  k := 100;
  f := 0.5438942180826245858e-292;
  y := poisson_pmfx(mu,k);
  testrel(11, 250, y, f, cnt,failed);
{$else}
  k := 0;
  f := 0.5075958897549456765e-434;
  y := poisson_pmfx(mu,k);
