...
begin
  ...
  else if x > +7.0 then sfc_erf := +1.0
  else if x < -7.0 then sfc_erf := -1.0
...
begin
  ...
  else if x < -7.0 then sfc_erfc := 2.0
  else if x > 106.8 then sfc_erfc := 0.0
  else begin
...
begin
  ...
  if y <= 1e-10 then sfc_erfce := 1.0 - 2.0*x/SqrtPi
    ...
    { -6.75 < x <= 128: Recycle sfc_erfc_iusc}
    sfc_erfc_iusc(y,y,z);
    y := y/z;
...
function sfc_erf_q(x: extended): extended;
  {-Return the probability function erf_q = integral(exp(-t^2/2)/sqrt(2*Pi), t=x..Inf)}
begin
  if IsNan(x) then sfc_erf_q := x
  else sfc_erf_q := sfc_erf_p(-x);  {HMF[1], 26.2.6}
end;
...
{---------------------------------------------------------------------------}
procedure fresnel_cfrac(x: extended; var fs,fc: extended);
...
{---------------------------------------------------------------------------}
procedure fresnel_large(x: extended; var fs,fc: extended);
...
const
  ...
  xlarge = 1.562e6;      {~ cbrt(4/Pi^2/eps_x)}
begin
  if IsNan(x) then begin
    s := NaN_x;
    c := NaN_x;
...
const
  ...
  ae3l: array[0..24] of extended = (
         ...
         -0.3e-19,
          0.1e-19);
const
  e3infh: THexExtW = ($6450,$301F,$4E26,$E49A,$3FFE);  {8.9297951156924921124E-1}
var
  funinf: extended absolute e3infh; {2/9*Pi*3^(1/2)/Gamma(2/3)}
  t,y: extended;
begin
  ...
  else if x <= 0.4e-6 then sfc_expint3 := x
  else if x <= 2.0 then begin
    t := x*sqr(0.5*x) - 1.0;
    sfc_expint3 := x*CSEvalX(t, ae3s, 25);
  end
  else if x > 3.6 then sfc_expint3 := funinf
  else begin
    y := x*sqr(x);
    t := CSEvalX(16.0/y - 1.0, ae3l, 25);
    y := exp(-y)/(3.0*sqr(x));
    sfc_expint3 := funinf - y*t;
  end;
end;
