...
int test_hermite(void)
{
  ...
  TEST_SF(s, gsl_sf_hermite_prob, (0, 0.75),  1.,  TEST_TOL0, GSL_SUCCESS);

  TEST_SF(s, gsl_sf_hermite_prob, (1, 0.75),  x,  TEST_TOL0, GSL_SUCCESS);

  n = 25;
  TEST_SF(s, gsl_sf_hermite_prob, (n, 0.),  0.,  TEST_TOL0, GSL_SUCCESS);
  n = 28;
  TEST_SF(s, gsl_sf_hermite_prob, (n, 0.),  213458046676875,  TEST_TOL0, GSL_SUCCESS);
  n = 25;
  TEST_SF(s, gsl_sf_hermite_prob, (n, 0.75),  -1.08128685847680748265939328423e12,  TEST_TOL0, GSL_SUCCESS);
  n = 28;
  TEST_SF(s, gsl_sf_hermite_prob, (n, 0.75),  -1.60620252094658918105511125135e14,  TEST_TOL0, GSL_SUCCESS);

  n = 10025;
  x = ((sqrt(2*n+1.)+aizero1/pow(8.*n,1/6.))/2)*M_SQRT2;
  TEST_SF(s, gsl_sf_hermite_prob, (n, x),  -3.3090527852387782540121569578e18961,  TEST_TOL0, GSL_SUCCESS);

  n = 10028;
  x = ((sqrt(2*n+1.)+aizero1/pow(8.*n,1/6.))/2)*M_SQRT2;
  TEST_SF(s, gsl_sf_hermite_prob, (n, x),  -7.515478445930242044360704363e18967,  TEST_TOL0, GSL_SUCCESS);

  n = 10025;
  x = (sqrt(2*n+1.)-(aizero1/pow(8.*n,1/6.))/2)*M_SQRT2;
  TEST_SF(s, gsl_sf_hermite_prob, (n, x),  4.1369269649092456235914193753e22243,  TEST_TOL0, GSL_SUCCESS);

  n = 10028;
  x = (sqrt(2*n+1.)-(aizero1/pow(8.*n,1/6.))/2)*M_SQRT2;
  TEST_SF(s, gsl_sf_hermite_prob, (n, x),  8.363694992558646923734666303e22250,  TEST_TOL0, GSL_SUCCESS);

  n = 10025;
  x = (sqrt(2*n+1.)-2*(aizero1/pow(8.*n,1/6.)))*M_SQRT2;
  TEST_SF(s, gsl_sf_hermite_prob, (n, x),  7.398863979737363164340057757e22273,  TEST_TOL0, GSL_SUCCESS);

  n = 10028;
  x = (sqrt(2*n+1.)-2*(aizero1/pow(8.*n,1/6.)))*M_SQRT2;
  TEST_SF(s, gsl_sf_hermite_prob, (n, x),  1.507131397474022356488976968e22281,  TEST_TOL0, GSL_SUCCESS);

  x = 0.75;

  n = 128;
  m = 225;
  TEST_SF(s, gsl_sf_hermite_prob_der, (m, n, x),  0.,  TEST_TOL0, GSL_SUCCESS);

  n = 128;
  m = 5;
  TEST_SF(s, gsl_sf_hermite_prob_der, (m, n, x),  -3.0288278964712702882066404e112,  TEST_TOL0, GSL_SUCCESS);
  ...
  TEST_SF(s, gsl_sf_hermite_prob_series, (n, x, res),  -4.0451066556993485405907339548e68,  TEST_TOL0, GSL_SUCCESS);

  // phys

  x = 0.75;

  TEST_SF(s, gsl_sf_hermite_phys, (0, 0.75),  1.,  TEST_TOL0, GSL_SUCCESS);

  TEST_SF(s, gsl_sf_hermite_phys, (1, 0.75),  2.*x,  TEST_TOL0, GSL_SUCCESS);

  n = 25;
  TEST_SF(s, gsl_sf_hermite_phys, (n, 0.),  0.,  TEST_TOL0, GSL_SUCCESS);
  n = 28;
  TEST_SF(s, gsl_sf_hermite_phys, (n, 0.),  3497296636753920000,  TEST_TOL0, GSL_SUCCESS);
  n = 25;
  TEST_SF(s, gsl_sf_hermite_phys, (n, 0.75),  -9.7029819451106077507781088352e15,  TEST_TOL0, GSL_SUCCESS);
  n = 28;
  TEST_SF(s, gsl_sf_hermite_phys, (n, 0.75),  3.7538457078067672096408339776e18,  TEST_TOL0, GSL_SUCCESS);

  n = 10025;
  x = ((sqrt(2*n+1.)+aizero1/pow(8.*n,1/6.))/2);
  TEST_SF(s, gsl_sf_hermite_phys, (n, x),  -2.7074282783315424535693575770e20470,  TEST_TOL0, GSL_SUCCESS);

  n = 10028;
  x = ((sqrt(2*n+1.)+aizero1/pow(8.*n,1/6.))/2);
  TEST_SF(s, gsl_sf_hermite_phys, (n, x),  -1.7392214893577690864150561850e20477,  TEST_TOL0, GSL_SUCCESS);

  n = 10025;
  x = (sqrt(2*n+1.)-(aizero1/pow(8.*n,1/6.))/2);
  TEST_SF(s, gsl_sf_hermite_phys, (n, x),  3.3847852473526979744366379542e23752,  TEST_TOL0, GSL_SUCCESS);

  n = 10028;
  x = (sqrt(2*n+1.)-(aizero1/pow(8.*n,1/6.))/2);
  TEST_SF(s, gsl_sf_hermite_phys, (n, x),  1.9355145738418079256435712027e23760,  TEST_TOL0, GSL_SUCCESS);

  n = 10025;
  x = (sqrt(2*n+1.)-2*(aizero1/pow(8.*n,1/6.)));
  TEST_SF(s, gsl_sf_hermite_phys, (n, x),  6.053663953512337293393128307e23782,  TEST_TOL0, GSL_SUCCESS);

  n = 10028;
  x = (sqrt(2*n+1.)-2*(aizero1/pow(8.*n,1/6.)));
  TEST_SF(s, gsl_sf_hermite_phys, (n, x),  3.487782358276961096026268141e23790,  TEST_TOL0, GSL_SUCCESS);

  x = 0.75;

  n = 128;
  m = 225;
  TEST_SF(s, gsl_sf_hermite_phys_der, (m, n, x),  0.,  TEST_TOL0, GSL_SUCCESS);

  n = 128;
  m = 5;
  TEST_SF(s, gsl_sf_hermite_phys_der, (m, n, x),  2.89461215568095657569833e132,  TEST_TOL0, GSL_SUCCESS);
  ...
  TEST_SF(s, gsl_sf_hermite_phys_series, (n, x, res),  1.07772223811696567390619566842e88,  TEST_TOL0, GSL_SUCCESS);

  x = 0.75;
  n = 28;
  TEST_SF(s, gsl_sf_hermite_func, (n, 0),  0.290371943657199641200016132937,  TEST_TOL0, GSL_SUCCESS);
  TEST_SF(s, gsl_sf_hermite_func, (n, x),  0.23526280808621240649319140441,  TEST_TOL0, GSL_SUCCESS);
  n = 100028;
  TEST_SF(s, gsl_sf_hermite_func, (n, x),  -0.02903467369856961147236598086,  TEST_TOL4, GSL_SUCCESS);

  n = 10025;
  x = ((sqrt(2*n+1.)+aizero1/pow(8.*n,1/6.))/2.5);
  TEST_SF(s, gsl_sf_hermite_func, (n, x),  -0.05301278920004176459797680403,  TEST_TOL3, GSL_SUCCESS);

  n = 10028;
  x = ((sqrt(2*n+1.)+aizero1/pow(8.*n,1/6.))/2.5);
  TEST_SF(s, gsl_sf_hermite_func, (n, x),  0.06992968509693993526829596970,  TEST_TOL3, GSL_SUCCESS);

  n = 10025;
  x = (sqrt(2*n+1.)-(aizero1/pow(8.*n,1/6.))/2.5);
  TEST_SF(s, gsl_sf_hermite_func, (n, x),  0.08049000991742150521366671021,  TEST_TOL4, GSL_SUCCESS);

  n = 10028;
  x = (sqrt(2*n+1.)-(aizero1/pow(8.*n,1/6.))/2.5);
  TEST_SF(s, gsl_sf_hermite_func, (n, x),  0.08048800667512084252723933250,  TEST_TOL4, GSL_SUCCESS);

  n = 10025;
  x = (sqrt(2*n+1.)-2.5*(aizero1/pow(8.*n,1/6.)));
  TEST_SF(s, gsl_sf_hermite_func, (n, x),  7.97206830806663013555068100e-6,  TEST_TOL4, GSL_SUCCESS);

  n = 10028;
  x = (sqrt(2*n+1.)-2.5*(aizero1/pow(8.*n,1/6.)));
  TEST_SF(s, gsl_sf_hermite_func, (n, x),  7.97188517397786729928465829e-6,  TEST_TOL4, GSL_SUCCESS);
  ...
  TEST_SF(s, gsl_sf_hermite_func_series, (n, x, res),  0.81717103529960997134154552556,  TEST_TOL0, GSL_SUCCESS);

  m = 5;
  n = 28;
  TEST_SF(s, gsl_sf_hermite_func_der, (0, n, x),  0.235262808086212406493191404,  TEST_TOL0, GSL_SUCCESS);
  TEST_SF(s, gsl_sf_hermite_func_der, (m, n, x),  4035.32788513029308540826835,  TEST_TOL0, GSL_SUCCESS);
  ...
  n = 17;
  for (m=1; m<=n/2; m++) {
    TEST_SF(s, gsl_sf_hermite_prob_zero, (n, m),  He17z[m-1],  TEST_TOL0, GSL_SUCCESS);
    // printf("he(%d,%d)= %.18e\n",n,m,r.val);
  }
  ...
  n = 18;
  for (m=1; m<=n/2; m++) {
    TEST_SF(s, gsl_sf_hermite_prob_zero, (n, m),  He18z[m-1],  TEST_TOL0, GSL_SUCCESS);
    // printf("he(%d,%d)= %.18e\n",n,m,r.val);
  }
  ...
  n = 23;
  for (m=1; m<=n/2; m++) {
    TEST_SF(s, gsl_sf_hermite_prob_zero, (n, m),  He23z[m-1],  TEST_TOL0, GSL_SUCCESS);
    // printf("he(%d,%d)= %.18e\n",n,m,r.val);
  }
  ...
  n = 24;
  for (m=1; m<=n/2; m++) {
    TEST_SF(s, gsl_sf_hermite_prob_zero, (n, m),  He24z[m-1],  TEST_TOL0, GSL_SUCCESS);
    // printf("he(%d,%d)= %.18e\n",n,m,r.val);
  }
  ...
  n = 17;
  for (m=1; m<=n/2; m++) {
    TEST_SF(s, gsl_sf_hermite_phys_zero, (n, m),  H17z[m-1],  TEST_TOL0, GSL_SUCCESS);
    // printf("h(%d,%d)= %.18e\n",n,m,r.val);
  }
  ...
  n = 18;
  for (m=1; m<=n/2; m++) {
    TEST_SF(s, gsl_sf_hermite_phys_zero, (n, m),  H18z[m-1],  TEST_TOL0, GSL_SUCCESS);
    // printf("h(%d,%d)= %.18e\n",n,m,r.val);
  }
  ...
  n = 23;
  for (m=1; m<=n/2; m++) {
    TEST_SF(s, gsl_sf_hermite_phys_zero, (n, m),  H23z[m-1],  TEST_TOL0, GSL_SUCCESS);
    // printf("h(%d,%d)= %.18e\n",n,m,r.val);
  }
  ...
  n = 24;
  for (m=1; m<=n/2; m++) {
    TEST_SF(s, gsl_sf_hermite_phys_zero, (n, m),  H24z[m-1],  TEST_TOL0, GSL_SUCCESS);
    // printf("h(%d,%d)= %.18e\n",n,m,r.val);
  }
