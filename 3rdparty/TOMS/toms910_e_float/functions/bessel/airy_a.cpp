...
static e_float AiryAi_Series::AtZero(const e_float& x)
{
  // Use the Taylor series expansion for |x| small.
  // http://functions.wolfram.com/BesselAiryStruveFunctions/AiryAi/06/01/01/

  const e_float x_cubed_over_nine = ef::pown(x, static_cast<INT64>(3)) / ef::nine();

  return   ( AiryUtil::Ai_and_AiPrime_OfZero().front()      * ef::hyperg_0f1(ef::two_third(),  x_cubed_over_nine))
         + ((AiryUtil::Ai_and_AiPrime_OfZero().back()  * x) * ef::hyperg_0f1(ef::four_third(), x_cubed_over_nine));
}

static e_float AiryAi_Series::AtTransitionPlus(const e_float& x)
{
  ...
  return (sqrt_x * ef::cyl_bessel_k(ef::third(), X)) * one_over_sqrt3_pi;
}

static e_float AiryAi_Series::AtTransitionMinus(const e_float& xm)
{
  ...
  return (ef::third() * sqrt_x) * (Jp1_3 + Jm1_3);
}

static e_float AiryAi_Series::AtInfinityPlus(const e_float& x)
{
  // Use the hypergeometric representation in exponential form using
  // hyperg_2f0 for large positive values of x.
  // http://functions.wolfram.com/BesselAiryStruveFunctions/AiryAi/06/02/01/01/

  static const e_float sixth       = ef::one()  / static_cast<INT32>(6);
  static const e_float five_sixths = ef::five() / static_cast<INT32>(6);
  
  const e_float sqrt_x = ef::sqrt(x);
  const e_float Xm     = -AiryUtil::make_zeta(sqrt_x);
  const e_float sum    = ef::hyperg_2f0(sixth, five_sixths, ef::one() / (Xm * static_cast<INT32>(2)));

  const e_float factor = ef::exp(Xm) / (ef::sqrt(sqrt_x) * (ef::sqrt_pi() * static_cast<INT32>(2)));

  return factor * sum;
}

static e_float AiryAi_Series::AtInfinityMinus(const e_float& xm)
{
  ...
  // http://functions.wolfram.com/BesselAiryStruveFunctions/AiryAi/06/02/01/02/
...
e_float ef::airy_a(const e_float& x)
{
  if(ef::iszero(x))
  {
    return AiryUtil::Ai_and_AiPrime_OfZero().front();
  }
  else
  {
    ...
    {
      return AiryAi_Series::AtZero(x);
    }
    else if(xx <= ef::hundred())
    {
      return ef::isneg(x) ? AiryAi_Series::AtTransitionMinus(xx)
                          : AiryAi_Series::AtTransitionPlus (x);
    }
    else
    {
      return ef::isneg(x) ? AiryAi_Series::AtInfinityMinus(xx)
                          : AiryAi_Series::AtInfinityPlus (x);
    }
  }
}
