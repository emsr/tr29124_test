...
/* Redefine type double to quad-double. */

#define double qd_real

/* Redefine pow() because in libqd it works only on integer exponent (???)
exp() and log() are optimized for quad-double though. */

#define pow(x,y) (exp((y)*log((x))))

/* Redefine floor() because in libqd its behavior is machine-dependent (???)
it works fine under Linux x86, but gives minus one correct result under SunOS
and Darwin. */

#define floor(x) ((x) >= 0 ? (double) (int) (x) : (double) (int) (x) -1)

/* Machine epsilon is quad-double... */

#define macheps qd_real::_eps

/* libqd doesn't have minimum machine number (???) */
