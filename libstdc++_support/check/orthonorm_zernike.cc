...
template<typename _Tp>
  ...
  norm_zernike(int n1, int m1, int n2, int m2, _Tp rho)
  {
    ...
    auto val
	= __gnu_cxx::integrate(fun, _Tp{0}, _Tp{_S_2pi}, _S_eps, _S_eps, 1024,
		    __gnu_cxx::Kronrod_21);
...
main()
{
  test_zernike<float>(1, 2);
  test_zernike<double>(1, 2);
  test_zernike<long double>(1, 2);
}
