...
template<typename _Tp>
  ...
  { // positive integer s
    ...
    std::cout << "Iterations in Integer Series: " << __j << '\n';
    return __res;
  }
 ...
 * and 
 * B_p(w) = - 2 (2 pi)^(p-1) * \sum \limits_{k = 0}^\infty \Gamma(2 + 2k - p)/ (2k+1)! (-1)^k (w/2/\pi)^(2k+1) (Zeta(2 + 2k - p) - _Tp{1})
...
template<typename _Tp, int __sigma>
  ...
  //std::cout << "Iterations in the series for s = -4n : " << __k << '\n';
  return __res;
}

/**
 * This function catches the cases of negative integer index s which are odd.
 ...
 * In the template parameter sigma we transport whether p = 1 + 4k (sigma = 1) or p = 3 + 4k  (sigma = -1)
...
template<typename _Tp, int __sigma>
  ...
  __polylog_exp_neg_odd(unsigned int __n, std::complex<_Tp> __w)
  {
	...
	* __pref * std::cos(_Tp(__np) * std::atan(std::complex<_Tp>{2 * _S_pi} / __w));
    ...
    //std::cout << "Iterations in the series for s = -(1+2*k) : " << __k << '\n';
    return __res;
}
...
template<typename _Tp>
  ...
  { // positive s
	...
	* std::exp(-std::lgamma(__s) + (__s - _Tp{1}) * std::log(-__w));
    ...
    const auto __pref = _Tp{2} * std::pow(_S_2pi, __s - 1);
    // now comes the remainder of the series
    ...
    while (!terminate) // Assume convergence.
      { // FIXME: optimize.
	int __idx = __m + 1 + __j;
	auto __zetaarg = 1 + __idx - __s;
	_Tp __sine;
...
template<typename _Tp>
  std::complex<_Tp>
  __polylog_exp_asym(const _Tp __s, std::complex<_Tp> __w)
  { // asymptotic expansion
	...
	//std::cout << __k << ' ' << __newterm << ' ' << std::endl;
...
template<typename PowerType, typename _Tp>
  ...
  __polylog_exp_negative_real_part(PowerType __s, _Tp __w)
  {
    ...
    std::cout << "iterations in __polylog_exp_negative_real_part: " << __k << std::endl;
    return __res;
}
...
template<typename _Tp>
  ...
  __polylog_exp_int_pos(const unsigned int __s, std::complex<_Tp> __w)
  {
	...
	if (__rw < -(_S_pi_2 + _S_pi / _Tp{5})  )
	  ...
	  return __polylog_exp_asym(static_cast<_Tp>(__s), __clamp_0_m2pi(__w));
      }
  }
...
template<typename _Tp>
  ...
  __polylog_exp_int_pos(const unsigned int __s, _Tp __w)
  {
	...
	if (__w < -(_S_pi_2 + _S_pi / _Tp{5}))
	  ...
	  // FIXME: The series should terminate after a finite number of terms.
	  return __polylog_exp_asym(static_cast<_Tp>(__s), std::complex<_Tp>(__w));
      }
  }
...
template<typename _Tp>
  ...
  __polylog_exp_real_pos(_Tp __s, std::complex<_Tp> __w)
  {
    ...
    if (__rw < _Tp{6}) // arbitrary transition point
      ...
      // Wikipedia says that this is required for Wood's formula
      return __polylog_exp_asym(__s, __clamp_0_m2pi(__w));
  }
...
template<typename _Tp>
  ...
  __polylog_exp_real_pos(_Tp __s, _Tp __w)
  {
    ...
    if (__w < _Tp{6}) // arbitrary transition point
      ...
      return __polylog_exp_asym(__s, std::complex<_Tp>(__w));
}
...
template<typename _Tp, typename ArgType>
  ...
  {
    if (__s > _Tp{25}) // Cutoff chosen by some testing on the real axis.
      return __polylog_exp_negative_real_part(__s, __w);
...
/**
 * A function to implement the __polylog in those cases where we can calculate it.
 * @param s The index s.
 * @param w A complex w.
...
template<typename _Tp>
  ...
  {
    if (__fpequal(std::imag(__w), _Tp{0}))
...
/**
 *  A function to implement __dirichlet's Eta function.
 *  @param w A w
...
/**
 * A function to implement __dirichlet's beta function.
 * @param w A w
 * @return if w lacks an imaginary part, we calculate the value, else we throw an exception.
 */
template<typename _Tp>
...
template<typename _Tp>
  inline _Tp
  __claussen_sl(unsigned int __m, std::complex<_Tp> __w)
  {
    constexpr auto _S_i = std::complex<_Tp>{0, 1};
