...
void   sv_decomp( double **a, int m, int n, double *w, double **v) {
    ...
    for ( k = n; k >= 1; --k ) {
        for ( its = 1; its <= ITS; ++its ) {
            ...
            if ( its == ITS ) nrerror( "No convergence in  30 sv_decomp iterations." );
