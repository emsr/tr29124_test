...
namespace __detail
{
  ...
  template<typename _Tp, typename _FuncTp, std::size_t __ksz, std::size_t __gsz>
    ...
    {
      const _Tp __center = 0.5 * (__a + __b);
      const _Tp __half_length = 0.5 * (__b - __a);
      const _Tp __abs_half_length = std::abs(__half_length);
      const _Tp __f_center = __func(__center);
      std::array<_Tp, __ksz> __fv1;
      std::array<_Tp, __ksz> __fv2;

      assert((__ksz/2) == __gsz);

      _Tp __result_gauss = 0;
      _Tp __result_kronrod = __f_center * __wgk[__ksz - 1];

      _Tp __result_abs = std::abs(__result_kronrod);
      _Tp __result_asc = 0;
      _Tp __mean = 0, __err = 0;
      ...
      for (__jj = 0; __jj < (__ksz - 1) / 2; ++__jj)
	{
	  ...
	  const _Tp __abscissa = __half_length * __xgk[__jtw];
	  const _Tp __fval1 = __func(__center - __abscissa);
	  const _Tp __fval2 = __func(__center + __abscissa);
	  const _Tp __fsum = __fval1 + __fval2;
	  __fv1[__jtw] = __fval1;
	  __fv2[__jtw] = __fval2;
      ...
      for (__jj = 0; __jj < __ksz / 2; ++__jj)
	{
	  ...
	  const _Tp __abscissa = __half_length * __xgk[__jtwm1];
	  const _Tp __fval1 = __func(__center - __abscissa);
	  const _Tp __fval2 = __func(__center + __abscissa);
  ...
  template<typename _Tp, typename _FuncTp>
    ...
    public:
      ...
      static void
      _S_integrate(const _FuncTp& func, _Tp a, _Tp b,
