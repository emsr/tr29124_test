	  ...
	  if (__iv.depth < 3)
	    {
	      ...
	      for (std::size_t __i = __skip[__d]; __i <= 32; __i += 2 * __skip[__d])
		__iv.fx[__i] = __func(__m + xi[__i] * __h);
	      __num_NaNs = 0;
		...
		if (std::isinf(__iv.fx[__i]))
	      ...
	      if (__num_NaNs > 0)
		{
		  downdate(&(__iv.c[__idx[__d]]), __n[__d], __d, __NaN, __num_NaNs);
		  for (std::size_t __i = 0; __i < __num_NaNs; ++__i)
		    __iv.fx[__NaN[__i]] = _S_NaN;
		}
		...
		{
		  auto __temp = __iv.c[__idx[__d - 1] + __i] - __iv.c[__idx[__d] + __i];
		  __ncdiff += __temp * __temp;
	  ...
	  if ((__m + __h * xi[0]) >= (__m + __h * xi[1])
	      ...
	      for (std::size_t __i = 0; __i <= __n[__d]; ++__i)
		{
		  ...
		  for (std::size_t __j = __i; __j <= __n[__d]; ++__j)
		    __ivl.c[__idx[__d] + __i] += Tleft[__i * 33 + __j] * __iv.c[__idx[__d] + __j];
		}
	      __ncdiff = _Tp{0};
