// $HOME/bin/bin/g++ -o test_lerch_transcendent test_lerch_transcendent.cpp -lquadmath

// LD_LIBRARY_PATH=$HOME/bin/lib64:$LD_LIBRARY_PATH ./test_lerch_transcendent > test_lerch_transcendent.txt

// g++ -std=c++14 -o test_lerch_transcendent test_lerch_transcendent.cpp -lquadmath
      ...
      else
	{
	  ...
	  for (auto __n = 1; __n < _S_maxit; ++__n)
	      ...
	      for (auto __k = 1; __k <= __n; ++__k)
		{
	      ...
	      __zfact *= __zfrac;
	      __lerch += __zfact * __term;
	      if (std::abs(__zfact * __term / __lerch) < _S_eps)
		return __sum;
	    }
	  __lerch /= (_Tp{1} - __z);
	  return __lerch;
