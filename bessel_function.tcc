...
namespace std _GLIBCXX_VISIBILITY(default)
{
  ...
  _GLIBCXX_BEGIN_NAMESPACE_VERSION
                          ...
                          _Tp & __Jnu, _Tp & __Nnu)
    {
      ...
      const _Tp __x8 = _Tp(8) * __x;
      _Tp __P = _Tp(1);
      _Tp __Q = (__mu - _Tp(1)) / __x8;
      unsigned int __k = 2;
      const _Tp __eps = std::numeric_limits<_Tp>::epsilon();
      _Tp __t = _Tp(1);
      do
        {
          _Tp __k2m1 = _Tp(2) * __k - _Tp(1);
          __t *= -(__mu - __k2m1 * __k2m1) / (__k * __x8);
          const bool __convP = std::abs(__t) < __eps * std::abs(__P);
          __P += __t;
          ++__k;

          __k2m1 = _Tp(2) * __k - _Tp(1);
          __t *= (__mu - __k2m1 * __k2m1) / (__k * __x8);
          const bool __convQ = std::abs(__t) < __eps * std::abs(__Q);
          __Q += __t;
          ++__k;
